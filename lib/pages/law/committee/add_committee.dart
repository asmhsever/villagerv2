...
class _CommitteeAddPageState extends State<CommitteeAddPage> {
  ...
  Future<void> _loadAvailableHouses() async {
    ...
    try {
      ...
      final available = houses.where((h) => !assignedHouseIds.contains(h.houseId)).toList();
        ...
        title: Row(
          children: [
            Container(
              ...
              child: const Icon(Icons.check_circle, color: oliveGreen, size: 24),
            ),
            const SizedBox(width: 12),
            Text('สร้างสำเร็จ', style: TextStyle(color: earthClay, fontSize: 18, fontWeight: FontWeight.bold)),
          ],
        ),
        content: Text(
          createdId == null
          ...
          style: const TextStyle(fontSize: 14, color: warmStone),
        ),
        actions: [
          Row(
            children: [
              Expanded(
                child: TextButton(
                  ...
                  child: const Text('เพิ่มอีก', style: TextStyle(color: burntOrange)),
                ),
              ),
              Expanded(
      ...
      builder: (context) => AlertDialog(
        ...
        title: Row(
          children: [
            Container(
              padding: const EdgeInsets.all(8),
              decoration: BoxDecoration(color: mutedBurntSienna.withOpacity(0.1), borderRadius: BorderRadius.circular(8)),
              child: const Icon(Icons.error_outline, color: mutedBurntSienna, size: 24),
            ),
            const SizedBox(width: 12),
            const Text('เกิดข้อผิดพลาด', style: TextStyle(color: clayOrange, fontSize: 18, fontWeight: FontWeight.bold)),
          ],
        ),
  ...
  Future<bool> _onWillPop() async {
    ...
    final result = await showDialog<bool>(
      ...
      builder: (context) => AlertDialog(
        ...
        content: const Text('คุณมีข้อมูลที่ยังไม่ได้บันทึก ต้องการออกจากหน้านี้หรือไม่?', style: TextStyle(color: warmStone, fontSize: 14)),
        actions: [
          TextButton(onPressed: () => Navigator.of(context).pop(false), child: const Text('ยกเลิก', style: TextStyle(color: warmStone))),
          ElevatedButton(
            onPressed: () => Navigator.of(context).pop(true),
            style: ElevatedButton.styleFrom(backgroundColor: mutedBurntSienna, foregroundColor: ivoryWhite, shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(12))),
  ...
  Widget build(BuildContext context) {
    return WillPopScope(
      ...
      child: Scaffold(
        backgroundColor: ivoryWhite,
        appBar: AppBar(
          title: const Text('เพิ่มคณะกรรมการใหม่', style: TextStyle(color: Colors.white, fontWeight: FontWeight.bold, fontSize: 18)),
          ...
          actions: [
            if (_selectedHouse != null && !_isLoading)
              IconButton(icon: const Icon(Icons.add, color: Colors.white), onPressed: _createCommittee, tooltip: 'เพิ่มคณะกรรมการ'),
          ],
        ),
        body: _isLoading
            ? Center(
          ...
          child: Form(
            ...
            child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              Container(
                ...
                child: Row(children: [
                  Container(
                    width: 60,
                    height: 60,
                    decoration: BoxDecoration(color: softBrown, borderRadius: BorderRadius.circular(30), boxShadow: [BoxShadow(color: softBrown.withOpacity(0.3), blurRadius: 8, offset: Offset(0, 2))]),
                    child: const Icon(Icons.group_add, color: Colors.white, size: 28),
                  ),
                  const SizedBox(width: 16),
                  Expanded(
                    child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                      Text('เพิ่มคณะกรรมการใหม่', style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold, color: earthClay)),
                      const SizedBox(height: 4),
                      Text('หมู่บ้าน: ${widget.villageId}', style: TextStyle(fontSize: 14, color: warmStone)),
                    ]),
                  ),
                ]),
              ),
              ...
              Container(
                ...
                decoration: BoxDecoration(color: oliveGreen.withOpacity(0.1), borderRadius: BorderRadius.circular(12), border: Border.all(color: oliveGreen.withOpacity(0.3), width: 1)),
                child: Row(children: [
                  const Icon(Icons.info_outline, color: oliveGreen, size: 20),
                  const SizedBox(width: 8),
                  Expanded(child: Text('มีบ้านที่สามารถเพิ่มคณะกรรมการได้: ${_availableHouses.length} บ้าน', style: const TextStyle(color: oliveGreen, fontSize: 14, fontWeight: FontWeight.w500))),
                ]),
              ),
              const SizedBox(height: 24),
              ...
              if (_availableHouses.isEmpty) ...[
                ...
                Container(
                  width: double.infinity,
                  padding: const EdgeInsets.all(16),
                  decoration: BoxDecoration(color: mutedBurntSienna.withOpacity(0.1), borderRadius: BorderRadius.circular(12), border: Border.all(color: mutedBurntSienna.withOpacity(0.3), width: 1)),
                  child: Column(children: const [
                    Icon(Icons.warning_amber, color: mutedBurntSienna, size: 32),
                    SizedBox(height: 8),
                    Text('ไม่มีบ้านที่สามารถเพิ่มคณะกรรมการได้', style: TextStyle(color: mutedBurntSienna, fontSize: 16, fontWeight: FontWeight.bold), textAlign: TextAlign.center),
                    SizedBox(height: 4),
                    Text('บ้านทั้งหมดในหมู่บ้านนี้มีคณะกรรมการแล้ว', style: TextStyle(color: Color(0xFFC8755A), fontSize: 12), textAlign: TextAlign.center),
                  ]),
                ),
              ],
              ...
              Row(children: [
                ...
                Expanded(
                  child: ElevatedButton.icon(
                    onPressed: (_selectedHouse != null && !_isLoading && _availableHouses.isNotEmpty) ? _createCommittee : null,
                    icon: const Icon(Icons.add),
                    label: const Text('เพิ่มคณะกรรมการ'),
                    style: ElevatedButton.styleFrom(
                      backgroundColor: _selectedHouse != null ? oliveGreen : warmStone,
                      foregroundColor: ivoryWhite,
    ...
    return Column(
      ...
      children: [
        ...
        Container(
          ...
          decoration: BoxDecoration(
            color: _availableHouses.isEmpty ? warmStone.withOpacity(0.1) : ivoryWhite,
            borderRadius: BorderRadius.circular(12),
            border: Border.all(
              color: _selectedHouse != null
                  ? oliveGreen
                  : _availableHouses.isEmpty
                  ? mutedBurntSienna.withOpacity(0.5)
                  : warmStone.withOpacity(0.5),
              width: 1.5,
            ),
          ),
          child: DropdownButtonHideUnderline(
            child: DropdownButton<HouseModel>(
              ...
              hint: Row(children: [
                Icon(Icons.home, color: _availableHouses.isEmpty ? mutedBurntSienna : warmStone, size: 20),
                const SizedBox(width: 12),
                Text(_availableHouses.isEmpty ? 'ไม่มีบ้านที่สามารถเลือกได้' : 'เลือกบ้าน...', style: TextStyle(color: _availableHouses.isEmpty ? mutedBurntSienna : warmStone, fontSize: 16)),
              ]),
              icon: Icon(Icons.arrow_drop_down, color: _availableHouses.isEmpty ? mutedBurntSienna : earthClay),
              isExpanded: true,
                  ...
                  child: Row(children: [
                    ...
                    Expanded(
                      child: Column(crossAxisAlignment: CrossAxisAlignment.start, mainAxisSize: MainAxisSize.min, children: [
                        Text('บ้านเลขที่: ${house.houseId}', style: TextStyle(fontSize: 16, color: earthClay, fontWeight: FontWeight.w600), overflow: TextOverflow.ellipsis),
                        if (house.houseNumber != null || house.owner != null)
                          Text('${house.houseNumber ?? 'ไม่ระบุหมายเลข'} • ${house.owner ?? 'ไม่ระบุเจ้าของ'}', style: TextStyle(fontSize: 12, color: warmStone), overflow: TextOverflow.ellipsis),
                      ]),
                    ),
                  ]),
                );
        ...
        if (_selectedHouse != null) ...[
          ...
          Container(
            ...
            child: Row(children: [
              ...
              Expanded(
                child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                  Text('เลือกแล้ว: บ้านเลขที่ ${_selectedHouse!.houseId}', style: const TextStyle(color: oliveGreen, fontSize: 12, fontWeight: FontWeight.w600)),
                  if (_selectedHouse!.owner != null)
                    Text('เจ้าของ: ${_selectedHouse!.owner}', style: TextStyle(color: oliveGreen.withOpacity(0.8), fontSize: 11)),
                ]),
              ),
            ]),
          ),
