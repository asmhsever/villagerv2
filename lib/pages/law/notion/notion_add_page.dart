...
class NotionAddPage extends StatefulWidget {
  const NotionAddPage({super.key});

  @override
  State<NotionAddPage> createState() => _NotionAddPageState();
}

class _NotionAddPageState extends State<NotionAddPage> {
  ...
  Future<void> _submit() async {
    ...
    if (user is! LawModel) {
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('ไม่สามารถระบุผู้ใช้ได้')));
      return;
    }
    ...
    await NotionDomain().create(notion);
